# bot_webui.py - Flask Web UI with EXACT original bot.py logic + Optimized Memory Management
...
def log_message(task_id: str, msg: str):
    ...
    if task_id not in task_logs:
        task_logs[task_id] = deque(maxlen=100)  # Only 100 logs per task
    
    task_logs[task_id].append(formatted_msg)
    
    # No file writing - saves memory
    print(formatted_msg)
...
class TaskManager:
    ...
    def _setup_browser(self, task_id: str):
        """EXACT SAME as original bot.py - WORKING VERSION"""
    ...
    def _send_messages(self, task: Task, process_id: str):
        """EXACT SAME send_messages function from original bot.py + Memory Cleanup"""
        driver = None
        task_id = task.task_id
        message_rotation_index = task.rotation_index
        ...
        try:
            ...
            log_message(task_id, f"{process_id}: Starting from message index: {message_rotation_index}")
            ...
            while task.status == "running" and not task.stop_flag:
                ...
                # Save rotation index after each message
                task.rotation_index = message_rotation_index
                ...
                try:
                    # EXACT ORIGINAL MESSAGE SENDING LOGIC
                    ...
                    log_message(task_id, f"{process_id}: Message #{messages_sent_in_session} (rotation: {message_rotation_index}) sent. Waiting {delay}s...")
                    
                    # MEMORY CLEANUP - Only clear cache, no browser restart
                    if messages_sent_in_session % 30 == 0:
                        log_message(task_id, f"{process_id}: Performing memory cleanup...")
                        try:
                            driver.execute_script("""
                                try {
                                    localStorage.clear();
                            ...
                            log_message(task_id, f"{process_id}: Cleanup error (ignored): {cleanup_error}")
